...
def judge_strike(score: Score, st: PlayState, s: Strike) -> tuple[PlayState, Judgement]:
    ...
    for i, n in enumerate(score.notes):
        ...
        if abs(off) <= MISS_MS and (best_off is None or abs(off) < abs(best_off)):
            best, best_off = i, off
    if best is None:
        return replace(st, extra=st.extra + 1, streak=0), Judgement(None, "extra", 0.0, s.finger)
    n = score.notes[best]
    if n.finger != s.finger:
        return replace(st, hit=st.hit | {best}, wrong=st.wrong + 1, streak=0), Judgement(best, "wrong_finger", best_off, s.finger)
    a = abs(best_off)
    if a <= PERFECT_MS:
        v, pts = "perfect", 100
    elif a <= GOOD_MS:
    ...
    return replace(st, hit=st.hit | {best}, streak=st.streak + 1, points=st.points + int(pts * mult),
                   perfect=st.perfect + (v == "perfect"), good=st.good + (v == "good")), Judgement(best, v, best_off, s.finger)
...
def sweep_misses(score: Score, st: PlayState, now_s: float) -> tuple[PlayState, list[int]]:
    ...
    if not missed:
        return st, []
    return replace(st, hit=st.hit | set(missed), misses=st.misses + len(missed), streak=0), missed
